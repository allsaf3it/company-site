            ...
            <div class="cb-rcfeed-content -lg">
                <div class="cb-rcfeed-container">
                    ...
                    <div class="cb-rcfeed-filter">
                        ...
                        @foreach($parentBlogs as $parentBlog)
                            <a class="cb-rcfeed-filter-item" href="#" data-filter-target="{{$parentBlog->id}}">
                                <span>{{app()->getLocale() == 'en' ? $parentBlog->title_blog_en : $parentBlog->title_blog_ar}}</span>
                            </a>
                        @endforeach
                    </div>
                </div>
                <div class="cb-rcfeed-container -ml">
                    <div class="cb-rcfeed-items">
                        @foreach($blogs as $blog)
                            <a class="cb-rcfeed-item" href="{{app()->getLocale() == 'en' ? LaravelLocalization::localizeUrl('blog/'.$blog->link_en) : LaravelLocalization::localizeUrl('blog/'.$blog->link_ar) }}" data-cat="{{$blog->parent_id}}">
                                <span class="cb-rcfeed-item-img">
                                    <img src="{{url('uploads/blogitems/source/' . $blog->image)}}" loading="lazy" alt>
                                </span>
                                <span class="cb-rcfeed-item-tag">
                                    <span class="cb-rcfeed-item-tag-item">{{app()->getLocale() == 'en' ? $blog->title_blog_en : $blog->title_blog_ar}}</span>
                                </span>
                                <span class="cb-rcfeed-item-title">{!! app()->getLocale() == 'en' ? \Illuminate\Support\Str::limit(strip_tags($blog->text_en), $limit = 50, $end = '...') : \Illuminate\Support\Str::limit(strip_tags($blog->text_ar), $limit = 50, $end = '...') !!}</span></a>
                        @endforeach
