...
class ConfigrationController extends Controller
{
    ...
    public function update(Request $request, $lang)
    {
        ...
        $configration -> projects_text = $request -> projects_text;
        $configration -> address1 = $request -> address1;
        $configration -> address2 = $request -> address2;
        $configration -> privacy_title = $request -> privacy_title;
        $configration -> privacy_text = $request -> privacy_text;
        ...
        if($request->hasFile("footer_video")){
            @unlink( base_path() . '/uploads/settings/source/'. $configration->footer_video);
            $extension = $request->file('footer_video')->getClientOriginalExtension();
            $filename = rand(11111111, 99999999). '.' . $extension;
            $request->file('footer_video')->move( base_path() . '/uploads/settings/source', $filename );
            $configration->footer_video = $filename;
        }
