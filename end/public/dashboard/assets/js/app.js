!(function () {
  ...
  function D() {
    ...
    e &&
      Array.from(e).forEach(function (o) {
        !(function e() {
          var dataTarget = o.getAttribute("data-target");
  
          if (dataTarget) {
            var targetValue = parseFloat(dataTarget.replace(",", ".")); // Replace comma with dot
  
            if (!isNaN(targetValue)) {
              var currentValue = parseFloat(o.innerText.replace(",", "."));
              var step =  currentValue / 50;
              step < 1 && (step = 1);
  
              if (currentValue < targetValue) {
                o.innerText = (currentValue + step).toFixed(2) ; // Add percentage symbol
                setTimeout(e, 1);
              } else {
                o.innerText = dataTarget; // Add percentage symbol
              }
            }
          }
        })();
  ...
  (document.getElementById("two-column-menu").innerHTML = ""),
    ...
    document.getElementById("scrollbar").classList.remove("h-100");
  var a = 6, // Change 'a' to 8 to display 8 links
